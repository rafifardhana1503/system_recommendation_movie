...
recommendations_movies = get_recommendations_with_scores(movie_title)

print(f"\nRekomendasi film untuk '{movie_title}' dengan perhitungan Cosine Similarity:")
print(recommendations_movies)

# Contoh penggunaan sistem rekomendasi 2 (dua) dengan skor
movie_title = "Harry Potter and the Prisoner of Azkaban"
recommendations_movies_2 = get_recommendations_with_scores(movie_title)

print(f"\nRekomendasi film untuk '{movie_title}' dengan perhitungan Cosine Similarity:")
print(recommendations_movies_2)
...
# Membuat fungsi untuk mengecek apakah semua keyword target genre ada di kolom tags
def is_genres_match(tags, target_genres):
    genres_set = set(str(tags).lower().split())
    return target_genres.issubset(genres_set)

# Membuat fungsi untuk mengambil relevant tags terhadap preferensi genre pengguna
def get_relevant_tags(tags, target_genres):
    genres_set = set(str(tags).lower().split())
    return genres_set.intersection(target_genres)

# Membuat fungsi untuk menghitung evaluasi precision
def evaluate_precision_by_genre(df_recommendations, df_full, target_genres):
    # Menggabungkan kolom "tags" dari df_final ke recommendations_df
    df_recommendations = df_recommendations.merge(
        df_full[['title', 'tags']], on='title', how='left')

    # Menandai kecocokan genres berdasarkan kolom tags
    df_recommendations['genres_match'] = df_recommendations['tags'].apply(
        lambda x: is_genres_match(x, target_genres))

    # Menghitung precision
    jumlah_film_relevan = df_recommendations['genres_match'].sum()
    jumlah_film_direkomendasikan = len(df_recommendations)
    precision = jumlah_film_relevan / jumlah_film_direkomendasikan
    precision_persen = precision * 100

    # Menampilkan hasil
    print(f"Hasil Precision berdasarkan rekomendasi: {precision:.2f} ({precision_persen:.1f}%)")
    print("\nEvaluasi per rekomendasi:")
    print(df_recommendations[['title', 'genres_match']])

    return precision, df_recommendations

# Membuat fungsi untuk melihat keseluruhan tags
def show_all_tags(df_recommendations, target):
    print(df_recommendations[['title', 'tags', 'genres_match']])

# Membuat fungsi untuk menyederhanakan tags (melihat relevant tags)
def show_relevant_tags(df_recommendations, target_genres):
    # Membuat kolom
    df_recommendations['relevant_tags'] = df_recommendations['tags'].apply(lambda x: get_relevant_tags(x, target_genres))
    df_recommendations['jumlah_match'] = df_recommendations['relevant_tags'].apply(len)

    # Menampilkan hasil
    print(df_recommendations[['title', 'relevant_tags', 'jumlah_match']])

"""1. **Evaluasi sistem rekomendasi 1 (satu)**\
Dengan pilihan film: Iron Man
"""

# Evaluasi penggunaan sistem rekomendasi 1 (satu)
target_genres = {'action', 'adventure', 'science', 'fiction'}
precision, df_recommendations = evaluate_precision_by_genre(
    df_recommendations = recommendations_movies,
    df_full = df_final,
    target_genres = target_genres)

# Melihat keseluruhan tags
show_all_tags(df_recommendations, target_genres)

# Melihat relevant tags terhadap genre preferensi pengguna
show_relevant_tags(df_recommendations, target_genres=target_genres)
...
2. **Evaluasi sistem rekomendasi 2 (dua)**\
Dengan pilihan film: Harry Potter and the Prisoner of Azkaban
"""

# Evaluasi penggunaan sistem rekomendasi 2
target_genres = {'adventure', 'fantasy', 'family'}
precision, df_recommendations = evaluate_precision_by_genre(
    df_recommendations = recommendations_movies_2,
    df_full = df_final,
    target_genres=target_genres)
